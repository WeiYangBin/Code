...
def train(epoch, dataset, config, log_dir):
    ...
    with tf.Graph().as_default():
        ...
        model.build_inference(is_train = 1)
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_train_batch:
                ...
                feed_dict = {
                    ...
                    # model.video_feature: np.sqrt(np.multiply(vgg, c3d)),
                    model.appear: vgg,
                    model.motion: c3d,
...
def val(epoch, dataset, config, log_dir):
    ...
    with tf.Graph().as_default():
        ...
        model = EMN(model_config)
        model.build_inference(is_train = 0)
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            acc = float(correct / dataset.val_example_total)
            print('\n[VAL] epoch {}, acc {:.5f}.\n'.format(epoch, acc))
...
def test(dataset, config, log_dir, ckpt_id):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ckpt_dir = os.path.join(log_dir, 'checkpoint')  #log/emn_msrvtt1/checkpoint
            # save_path = tf.train.latest_checkpoint(ckpt_dir) 
            # saver = tf.train.Saver()
            # if save_path:
            #     print('load checkpoint {}.'.format(save_path))
            #     saver.restore(sess, 'log/emn_dynamic/checkpoint/model.ckpt-14')#save_path
            # else:
            #     print('no checkpoint.')
            #     exit()
            
            if ckpt_id:
                ckpt_path = os.path.join(ckpt_dir, 'model.ckpt'+'-'+str(ckpt_id))
            else:
                ckpt_path = tf.train.latest_checkpoint(ckpt_dir)
            saver = tf.train.Saver()
            if ckpt_path:
            ...
            acc = float(correct / dataset.test_example_total)
            print('\n[TEST] acc {:.5f}.\n'.format(acc))
...
def main():
    ...
    if args.mode == 'train':
        ...
        for epoch in range(0, 50):
            ...
            if not_improved == 10:
                print('early stopping.')
                test(dataset, config, log_dir, epoch-10)
                break

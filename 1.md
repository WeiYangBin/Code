        ...
        with tf.variable_scope('memory'):
            ...
            with tf.variable_scope('episodic_appear') as scope:
                ...
                for p in range(3):
                    ...
                    att.append(attention(memory_appear, bool(p)))

                    # initialize GRU cell for RNN which returns final episodic memory
                    gru = tf.nn.rnn_cell.GRUCell(num_units=self.memory_dim, reuse=bool(p))
                    ...
                    def body(state, i):
                        state = att[-1][:,i,:] * gru(bi_appear[:,i,:], state)[0] + (1 - att[-1][:,i,:]) * state
                        return state, i + 1

                    # get episode by applying GRU attention
                    episode = tf.while_loop(cond=valid, body=body, loop_vars=[memory_appear, 0])[0]

                    # initialize weights for updating between episodes
                    w_epizode = tf.Variable(tf.truncated_normal([3 * self.memory_dim, self.memory_dim], stddev=0.05))
                    b_epizode = tf.Variable(tf.constant(0.01, shape=[self.memory_dim]))

                    m = tf.concat([memory_appear, episode, u1], 1)
                    ...
                    # if is_train == 1:
                    #     memory_appear = tf.nn.relu(m)
                    #     memory_appear = tf.nn.dropout(memory_appear, keep_prob=0.7)        
                    # if is_train == 0:
                    #     memory_appear = tf.nn.relu(m)
            ...
            with tf.variable_scope('episodic_motion') as scope:
                ...
                for p in range(3):
                    # get attention from memory (and question and facts which are defined before)
                    att.append(attention(memory_motion, bool(p)))
                    ...
                    # in each step update state with attention (how much to keep from past)
                    def body(state, i):
                        state = att[-1][:,i,:] * gru(bi_motion[:,i,:], state)[0] + (1 - att[-1][:,i,:]) * state
                        return state, i + 1

                    # get episode by applying GRU attention
                    episode = tf.while_loop(cond=valid, body=body, loop_vars=[memory_motion, 0])[0]

                    # initialize weights for updating between episodes
                    w_epizode = tf.Variable(tf.truncated_normal([3 * self.memory_dim, self.memory_dim], stddev=0.05))
                    b_epizode = tf.Variable(tf.constant(0.01, shape=[self.memory_dim]))

                    m = tf.concat([memory_motion, episode, u1], 1)
                    m = tf.matmul(m, w_epizode) + b_epizode
                    ...
                    # if is_train == 1:
                    #     memory_motion = tf.nn.relu(m)
                    #     memory_motion = tf.nn.dropout(memory_motion, keep_prob=0.7)
                    # if is_train == 0:
                    #     memory_motion = tf.nn.relu(m) 
            ...
            if is_train == 1:
                ...
                fuse = tf.nn.dropout(fuse, keep_prob = 0.5)
            if is_train == 0:
                fuse = tf.multiply(fuse, u1)
